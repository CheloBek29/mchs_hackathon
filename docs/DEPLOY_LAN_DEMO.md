...
VITE_RADIO_WS_URL=ws://192.168.1.10:8080/ws
```
...
- `wss://sim.local/radio/ws` -> radio server (`radio 2`)
...
VITE_RADIO_WS_URL=wss://sim.local/radio/ws
```

## 6.1 Быстрый сценарий для macOS (пункт 2)

1) На всех 4 Mac добавить host (`/etc/hosts`):

```bash
sudo sh -c 'printf "\n192.168.1.10 sim.local\n" >> /etc/hosts'
```

2) На хосте проекта положить `Caddyfile` в корень `mchs-simulator-submission/` (уже добавлен в пакет) и запустить Caddy:

```bash
cd mchs-simulator-submission
caddy run --config Caddyfile
```

3) На клиентах доверить root-сертификат Caddy (иначе микрофон будет блокироваться как небезопасный сайт):

- Скопировать с хоста файл:
  - `~/.local/share/caddy/pki/authorities/local/root.crt`
- Импортировать на Mac:
...
- открыть `https://sim.local`
- в Safari/Chrome выдать доступ к микрофону
- убедиться, что в адресной строке нет предупреждения о недоверенном сертификате
