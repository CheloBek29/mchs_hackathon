...
// Maximum distance (in world meters) a vehicle can be from a hose start to auto-link
const MAX_HOSE_REACH_M = 400;
...
const getVisibleDeployments = (deployments: ResourceDeploymentDto[]): ResourceDeploymentDto[] => {
  const supersededDeploymentIds = new Set<string>();
  deployments.forEach((deployment) => {
    const resourceData = deployment.resource_data;
    if (!resourceData || typeof resourceData !== 'object') {
      return;
    }
    const previousDeploymentId = (resourceData as Record<string, unknown>).previous_deployment_id;
    if (typeof previousDeploymentId === 'string' && previousDeploymentId.trim().length > 0) {
      supersededDeploymentIds.add(previousDeploymentId.trim());
    }
  });
  ...
  deployments.forEach((deployment) => {
    if (supersededDeploymentIds.has(deployment.id)) {
      return;
    }
...
const roleLabelFromRoleTag = (roleTag: string, fallback: string): string => {
  if (roleTag === 'BU1') {
    return 'БУ - 1';
  }
  if (roleTag === 'BU2') {
...
const canManageDeploymentForRole = (deployment: ResourceDeploymentDto, roleView: RoleView): boolean => {
  ...
  const deploymentRoleTag = getDeploymentRoleTag(deployment);
  const resourceData = toRecord(deployment.resource_data);
  const isPlanOnly = resourceData?.plan_only === true;

  if (roleView === 'RTP') {
    if (deploymentRoleTag === 'BU1' || deploymentRoleTag === 'BU2') {
      return false;
    }
    if (isPlanOnly && deploymentRoleTag === 'HQ') {
      return false;
    }
    return true;
  }
  ...
  if (roleView === 'HQ') {
    return isPlanOnly && (deploymentRoleTag === 'HQ' || deploymentRoleTag.length === 0);
  }
...
export const SimulationBoard: React.FC<SimulationBoardProps> = ({ activeRole, isReadOnly = false }) => {
  ...
  const prevNozzleDeploymentIdRef = useRef<string | null>(null);
  ...
  const canMoveSelectedDeployment = useMemo(() => {
    if (!selectedDeployment) {
      return false;
    }
    if (selectedDeployment.status === 'COMPLETED') {
    ...
    if (selectedDeployment.resource_kind !== 'HOSE_LINE' && selectedDeployment.resource_kind !== 'NOZZLE') {
      return false;
    }
    if (isBuRole && !isBuActivatedByRtp) {
      return false;
  ...
  const isMovePendingForSelected = useMemo(() => {
    if (!selectedDeployment || !pendingPlacement || pendingPlacement.source !== 'simulation_board_move') {
      return false;
    }
    const data = pendingPlacement.resourceData;
    if (!data || typeof data !== 'object') {
      return false;
    }
    return (data as Record<string, unknown>).previous_deployment_id === selectedDeployment.id;
  }, [pendingPlacement, selectedDeployment]);
  ...
  useEffect(() => {
    if (!selectedDeployment || selectedDeployment.resource_kind !== 'NOZZLE') {
      prevNozzleDeploymentIdRef.current = null;
      return;
    }
    // Only reset settings when a DIFFERENT nozzle is selected, not on every bundle update
    if (prevNozzleDeploymentIdRef.current === selectedDeployment.id) {
      return;
    }
    prevNozzleDeploymentIdRef.current = selectedDeployment.id;
  ...
  const requestSelectedDeploymentMove = useCallback(() => {
    if (!selectedDeployment || !canMoveSelectedDeployment) {
      return;
    }

    const selectedRoleTag = getDeploymentRoleTag(selectedDeployment);
    const roleLabel = roleLabelFromRoleTag(selectedRoleTag, activeRole ?? 'ОПЕРКАРТА');
    const currentData = toRecord(selectedDeployment.resource_data) ?? {};

    if (isMovePendingForSelected) {
      clearPendingPlacement();
      setHoverPlacementPoint(null);
      setLinePlacementStart(null);
      return;
    ...
    setPendingPlacement({
      resourceKind: selectedDeployment.resource_kind,
      geometryType: selectedDeployment.geometry_type,
      label: selectedDeployment.label,
      status: selectedDeployment.status,
      source: 'simulation_board_move',
      roleLabel,
      vehicleId: selectedDeployment.vehicle_dictionary_id ?? undefined,
      resourceData: {
        ...currentData,
        source: 'simulation_board_move',
        role: selectedRoleTag || currentData.role || roleLabel,
    ...
    showTransientMessage(
      `${roleLabel}: выберите ${selectedDeployment.geometry_type === 'LINESTRING' ? 'новую линию' : 'новую точку'} для ${selectedDeployment.label.toUpperCase()}`,
    );
  }, [
    activeRole,
    canMoveSelectedDeployment,
    clearPendingPlacement,
    isMovePendingForSelected,
    selectedDeployment,
    setPendingPlacement,
    showTransientMessage,
  ]);
  ...
  const handleCanvasClick = async (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const isMoveOperation =
      baseResourceData.move_resource === true
      && typeof baseResourceData.previous_deployment_id === 'string'
      && baseResourceData.previous_deployment_id.trim().length > 0;

    if (!isMoveOperation && pendingPlacement.resourceKind === 'HOSE_LINE') {
      ...
      const selectedVehicleCenter = extractGeometryCenter(selectedVehicle.geometry_type, selectedVehicle.geometry);
      if (!selectedVehicleCenter || Math.sqrt(distanceSquared(selectedVehicleCenter, lineStart)) > MAX_HOSE_REACH_M) {
        const dist = selectedVehicleCenter ? Math.round(Math.sqrt(distanceSquared(selectedVehicleCenter, lineStart))) : '?';
        setPlacementError(`Машина слишком далеко (${dist} м). Рукав должен начинаться у машины (макс. ${MAX_HOSE_REACH_M} м)`);
        return;
      }
    ...
    if (!isMoveOperation && pendingPlacement.resourceKind === 'HOSE_SPLITTER') {
      const roleTag = normalizeDeploymentRoleTag(baseResourceData.role ?? pendingPlacement.roleLabel);
      const vehicleCandidates = visibleDeployments
        .filter((deployment) => deployment.resource_kind === 'VEHICLE')
        .filter((deployment) => deployment.status === 'DEPLOYED' || deployment.status === 'ACTIVE')
      ...
      if (splitterVehicle?.vehicle_dictionary_id) {
        const splitterVehicleCenter = extractGeometryCenter(splitterVehicle.geometry_type, splitterVehicle.geometry);
        if (!splitterVehicleCenter || Math.sqrt(distanceSquared(splitterVehicleCenter, worldPoint)) > MAX_HOSE_REACH_M) {
          const dist = splitterVehicleCenter ? Math.round(Math.sqrt(distanceSquared(splitterVehicleCenter, worldPoint))) : '?';
          setPlacementError(`Машина слишком далеко для разветвления (${dist} м). Макс. ${MAX_HOSE_REACH_M} м`);
          return;
        }
    ...
    if (!isMoveOperation && pendingPlacement.resourceKind === 'NOZZLE') {
      const roleTag = normalizeDeploymentRoleTag(baseResourceData.role ?? pendingPlacement.roleLabel);
  ...
  return (
    <div className="relative w-full h-full bg-[#111] overflow-hidden">
      {!isReadOnly ? (
        <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-2 max-w-[55%] max-h-[calc(100%-7rem)] overflow-y-auto custom-scrollbar pb-1">
          ...
          <div className="bg-[#1b1b1b] border-2 border-black px-2 py-2 text-[7px] text-gray-100 w-[320px] max-w-full">
              ...
              <div className="mt-2 border border-black/60 bg-black/30 px-2 py-2">
                ...
                {!canAdjustSelectedNozzle ? (
                  <div className="mt-1 text-[6px] text-amber-300 uppercase leading-relaxed">
                    Недостаточно прав для изменения параметров этого ствола
                  </div>
                ) : null}
              </div>
            ) : null}
            <div className="mt-2 flex items-center gap-2">
              ...
              {canMoveSelectedDeployment ? (
                <PixelButton
                  size="sm"
                  variant={isMovePendingForSelected ? 'active' : 'default'}
                  className="text-[6px] px-2"
                  onClick={requestSelectedDeploymentMove}
                >
                  {isMovePendingForSelected ? 'ВЫБОР ТОЧКИ...' : 'ПЕРЕМЕСТИТЬ'}
                </PixelButton>
              ) : null}
              {selectedDeploymentAction ? (
                <PixelButton
      ...
      {viewportMode === 'SIM_25D' ? (
        ...
        <>
          ...
          {shouldRenderMiniMap ? (
            <div
              className="pointer-events-none absolute bottom-24 right-4 z-[6] max-w-[calc(40%-1rem)]"
