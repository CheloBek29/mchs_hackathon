...
const parseFireRuntimeSnapshot = (snapshotData: Record<string, unknown> | null): ParsedFireRuntime => {
  ...
  const hoseRuntimeRaw = toRecord(fireRuntime.hose_runtime);
  if (hoseRuntimeRaw) {
    Object.entries(hoseRuntimeRaw).forEach(([deploymentId, value]) => {
  ...
  const nozzleRuntimeRaw = toRecord(fireRuntime.nozzle_runtime);
  if (nozzleRuntimeRaw) {
    Object.entries(nozzleRuntimeRaw).forEach(([deploymentId, value]) => {
      const runtimeState = toRecord(value);
      if (!runtimeState) {
        return;
      }
  ...
  const vehicleRuntimeRaw = toRecord(fireRuntime.vehicle_runtime);
  if (!vehicleRuntimeRaw) {
    return parsed;
  }

  Object.entries(vehicleRuntimeRaw).forEach(([key, value]) => {
...
export const SimulationBoard: React.FC<SimulationBoardProps> = ({ activeRole, isReadOnly }) => {
    ...
    const vehicleCandidates = Array.from(latestVisibleVehicleDeployments.entries())
      .map(([vehicleId, deployment]) => {
        ...
        return {
          ...
          waterRemainingL: runtimeState?.waterRemainingL ?? 0,
        };
      })
  ...
  const selectedDeploymentRuntimeDetails = useMemo(() => {
    ...
    if (selectedDeployment.resource_kind === 'NOZZLE') {
      ...
      if (!isWet && runtimeState?.blockedReason) {
        ...
        details.push(`причина: ${reasonMap[runtimeState.blockedReason] ?? runtimeState.blockedReason.toLowerCase()}`);
      }
    }
  ...
  const getCanvasLocalPoint = useCallback((event: React.MouseEvent<HTMLCanvasElement>): Point => {
    ...
    (localPoint: Point): ResourceDeploymentDto | null => {
      ...
      for (const deployment of deploymentsByPriority) {
        ...
        if (deployment.geometry_type === 'POINT') {
          if (distanceSquared(localPoint, screenPoints[0]) <= 10 * 10) {
            return deployment;
          }
          continue;
        }
        ...
        if (deployment.geometry_type === 'LINESTRING') {
          if (screenPoints.length >= 2 && isPointNearPolyline(localPoint, screenPoints, 8)) {
            return deployment;
          }
          continue;
        }
  ...
  const drawBoard = useCallback(() => {
    ...
    hazardsToDraw.forEach((hazard) => {
      ...
      if (hazard.geometryType === 'POINT') {
        ...
        const baseRadius = clamp(radiusMeters * pixelsPerMeter, isSmoke ? 14 : 10, isSmoke ? 320 : 240);
        const pulse = hazard.isActive ? 1 + Math.sin((nowTickMs + point.x * 7) / (isSmoke ? 1100 : 420)) * (isSmoke ? 0.06 : 0.14) : 0.9;
        ...
        if (isSmoke) {
          ...
          for (let index = 0; index < 4; index += 1) {
            const phase = nowTickMs / 1200 + index * 1.35;
            const driftX = Math.cos(phase) * radiusPx * 0.28;
            const driftY = Math.sin(phase * 1.2) * radiusPx * 0.22;
            const blobRadius = radiusPx * (0.24 + index * 0.08);
            ctx.fillStyle = `rgba(71, 85, 105, ${0.13 + index * 0.03})`;
            ctx.beginPath();
          ...
          for (let index = 0; index < 5; index += 1) {
            ...
            const flameX = point.x + Math.cos(phase) * radiusPx * 0.22;
            const flameY = point.y + Math.sin(phase * 1.6) * radiusPx * 0.18;
            const flameRadius = radiusPx * (0.16 + ((index % 3) * 0.05));
            ctx.fillStyle = index % 2 === 0 ? 'rgba(251, 146, 60, 0.34)' : 'rgba(254, 215, 170, 0.3)';
            ctx.beginPath();
  ...
  const handleCanvasClick = async (event: React.MouseEvent<HTMLCanvasElement>) => {
    if (isReadOnly || suppressNextClickRef.current) {
    ...
    <div className="relative w-full h-full bg-[#111] overflow-hidden">
        ...
        <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-2 max-w-[55%]">
          ...
          {showAreaStats ? (
            <PixelButton variant="default" className="text-[7px] px-3 py-1 tracking-wide">
              ПЛОЩАДЬ: ОГОНЬ {Math.round(fireStats.activeFireAreaM2)} м2 | ДЫМ {Math.round(fireStats.activeSmokeAreaM2)} м2
            </PixelButton>
          ) : null}
      ...
      <div ref={canvasHolderRef} className="absolute inset-3 top-16 border-2 border-black bg-[#151515] p-2 overflow-hidden">
        <canvas
          ...
          className={`w-full h-full border border-[#2f2f2f] bg-[#111] ${isPanning ? 'cursor-grabbing' : pendingPlacement ? 'cursor-crosshair' : 'cursor-grab'
            }`}
