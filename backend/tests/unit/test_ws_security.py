...
def test_deployment_workflow_allows_hq_plan_only_vehicle_marker() -> None:
    hq_user = make_user_with_roles("HQ")
    assert_deployment_workflow_allowed_for_role(
        hq_user,
        ResourceKind.VEHICLE,
        DeploymentStatus.DEPLOYED,
        {"role": "HQ", "plan_only": True},
    )


def test_deployment_workflow_allows_rtp_tactical_hose_and_nozzle() -> None:
    rtp_user = make_user_with_roles("RTP")
    assert_deployment_workflow_allowed_for_role(
        rtp_user,
        ResourceKind.HOSE_LINE,
        DeploymentStatus.ACTIVE,
    ...
    assert_deployment_workflow_allowed_for_role(
        ...
        ResourceKind.NOZZLE,
        DeploymentStatus.ACTIVE,
        {"role": "RTP", "pressure": 60, "spray_angle": 10},
    )
