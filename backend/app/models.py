...
class SimulationSession(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "map_scale IS NULL OR map_scale > 0",
            name="ck_simulation_sessions_map_scale_positive",
        ),
        CheckConstraint(
            "time_multiplier > 0",
    ...
    radio_transmissions: Mapped[list["RadioTransmission"]] = relationship(
        back_populates="session", cascade="all, delete-orphan"
    )
...
class User(Base):
    ...
    admin_lock_owned: Mapped["SystemAdminLock | None"] = relationship(
        ...
        uselist=False,
        foreign_keys="SystemAdminLock.admin_user_id",
    )
    auth_sessions: Mapped[list["Session"]] = relationship(
...
class WeatherSnapshot(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "wind_speed >= 0", name="ck_weather_snapshots_wind_speed_non_negative"
        ),
        CheckConstraint(
            "wind_dir >= 0 AND wind_dir <= 359",
            name="ck_weather_snapshots_wind_dir_range",
        ),
        CheckConstraint(
    ...
    state_snapshot: Mapped[SessionStateSnapshot] = relationship(
...
class FireObject(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "area_m2 IS NULL OR area_m2 >= 0", name="ck_fire_objects_area_non_negative"
        ),
        CheckConstraint(
            "perimeter_m IS NULL OR perimeter_m >= 0",
            name="ck_fire_objects_perimeter_non_negative",
        ),
        CheckConstraint(
...
class ResourceDeployment(Base):
    ...
    vehicle: Mapped[VehicleDictionary | None] = relationship(
        back_populates="deployments"
    )
...
class RadioTransmission(Base):
    __tablename__ = "radio_transmissions"
    __table_args__ = (
        CheckConstraint(
            "duration_ms IS NULL OR duration_ms >= 0",
            name="ck_radio_transmissions_duration_non_negative",
        ),
        CheckConstraint(
            "chunk_index IS NULL OR chunk_index >= 0",
            name="ck_radio_transmissions_chunk_index_non_negative",
        ),
    )
    ...
    snapshot_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("session_state_snapshots.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    transmission_id: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    chunk_index: Mapped[int | None] = mapped_column(Integer, nullable=True)
    is_live_chunk: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    mime_type: Mapped[str] = mapped_column(
        String(64), nullable=False, default="audio/webm"
    )
    ...
    session: Mapped[SimulationSession] = relationship(
        back_populates="radio_transmissions"
    )
    ...
    def __str__(self) -> str:
        return f"Radio[{self.channel}] {self.sender_username} ({self.transmission_id})"
