...
def assert_deployment_workflow_allowed_for_role(
    ...
    if UserRole.HQ in user_roles:
        ...
        if resource_kind not in {
            ...
            raise HTTPException(
                status_code=403,
                detail="HQ planning supports vehicles, markers, hose lines/splitters, nozzles and water sources",
            )
        if status_value not in {
            ...
            raise HTTPException(
                status_code=403,
                detail="HQ planning resources must use PLANNED/DEPLOYED/ACTIVE/COMPLETED status",
            )
        return
    ...
    if UserRole.RTP in user_roles:
        if resource_kind not in {
            ...
            raise HTTPException(
                status_code=403,
                detail="RTP cannot place this resource type",
            )

        if role_tag and role_tag != UserRole.RTP.value:
            raise HTTPException(
                status_code=403,
                detail="RTP can manage only RTP-tagged tactical resources",
            )

        if resource_kind == ResourceKind.MARKER:
            command_point = str(resource_data.get("command_point") or "").strip().upper()
            if command_point and command_point not in {"HQ", "BU1", "BU2"}:
                raise HTTPException(
                    status_code=403,
                    detail="RTP marker command_point must be one of: HQ, BU1, BU2",
                )
        return
...
def resolve_previous_deployment_to_supersede(
    db,
    snapshot: SessionStateSnapshot,
    user: User,
    resource_kind: ResourceKind,
    resource_data: dict[str, Any],
) -> ResourceDeployment | None:
    raw_previous_id = resource_data.get("previous_deployment_id")
    if raw_previous_id is None or str(raw_previous_id).strip() == "":
        return None

    previous_deployment_id = parse_uuid(
        raw_previous_id, "resource_data.previous_deployment_id"
    )
    previous_deployment = db.get(ResourceDeployment, previous_deployment_id)
    if previous_deployment is None or previous_deployment.state_id != snapshot.id:
        raise HTTPException(
            status_code=404,
            detail="Previous deployment not found in current session snapshot",
        )

    if previous_deployment.resource_kind != resource_kind:
        raise HTTPException(
            status_code=409,
            detail="previous_deployment_id resource_kind mismatch",
        )

    previous_resource_data = (
        previous_deployment.resource_data
        if isinstance(previous_deployment.resource_data, dict)
        else {}
    )
    previous_role_tag = normalize_resource_role_tag(
        previous_resource_data.get("role")
        or previous_resource_data.get("initiated_from_role")
    )
    next_role_tag = normalize_resource_role_tag(
        resource_data.get("role") or resource_data.get("initiated_from_role")
    )
    if previous_role_tag and next_role_tag and previous_role_tag != next_role_tag:
        raise HTTPException(
            status_code=403,
            detail="previous_deployment_id role mismatch",
        )
    ...
    if UserRole.HQ in user_roles and previous_resource_data.get("plan_only") is not True:
        raise HTTPException(
            status_code=403,
            detail="HQ can supersede only plan_only resources",
        )

    if (
        UserRole.COMBAT_AREA_1 in user_roles
        and previous_role_tag != UserRole.COMBAT_AREA_1.value
    ):
        raise HTTPException(
            status_code=403,
            detail="BU-1 can supersede only BU-1 resources",
        )

    if (
        UserRole.COMBAT_AREA_2 in user_roles
        and previous_role_tag != UserRole.COMBAT_AREA_2.value
    ):
        raise HTTPException(
            status_code=403,
            detail="BU-2 can supersede only BU-2 resources",
        )

    if UserRole.RTP in user_roles and previous_role_tag in {
        UserRole.COMBAT_AREA_1.value,
        UserRole.COMBAT_AREA_2.value,
    }:
        raise HTTPException(
            status_code=403,
            detail="RTP cannot supersede BU resources",
        )

    return previous_deployment


def supersede_previous_deployment(
    previous_deployment: ResourceDeployment,
    next_deployment: ResourceDeployment,
    user: User,
) -> None:
    previous_data = (
        clone_json_dict(previous_deployment.resource_data)
        if isinstance(previous_deployment.resource_data, dict)
        else {}
    )
    previous_data["superseded_by_deployment_id"] = str(next_deployment.id)
    previous_data["superseded_at"] = utcnow().isoformat()
    previous_data["superseded_by_user_id"] = str(user.id)
    previous_deployment.status = DeploymentStatus.COMPLETED
    previous_deployment.resource_data = previous_data
...
def apply_fire_dynamics_tick(
    ...
    for deployment in deployments:
        ...
        deployment_resource_data = (
            deployment.resource_data if isinstance(deployment.resource_data, dict) else {}
        )
        if deployment_resource_data.get("plan_only") is True:
            continue
...
def apply_create_resource_deployment_command(
    ...
    previous_deployment = resolve_previous_deployment_to_supersede(
        db=db,
        snapshot=snapshot,
        user=user,
    ...
    if previous_deployment is not None:
        resource_data["previous_deployment_id"] = str(previous_deployment.id)
    ...
    if previous_deployment is not None:
        supersede_previous_deployment(previous_deployment, deployment, user)
