...
from .schemas import (
    ...
    FireObjectRead,
    FireObjectUpdate,
    LessonLlmEvaluationRead,
    LessonLlmEvaluationRequest,
    RefreshTokenRequest,
    RadioTransmissionRead,
...
refresh_rate_limit = rate_limit_dependency(
    "auth_refresh", max_requests=20, window_seconds=60
)

LLM_EVAL_BASE_URL = os.getenv("LLM_EVAL_BASE_URL", "").strip()
LLM_EVAL_API_KEY = os.getenv("LLM_EVAL_API_KEY", "lm-studio").strip() or "lm-studio"
LLM_EVAL_MODEL = os.getenv("LLM_EVAL_MODEL", "").strip()
LLM_EVAL_PROVIDER = (
    os.getenv("LLM_EVAL_PROVIDER", "lmstudio_openai_compatible").strip()
    or "lmstudio_openai_compatible"
)
try:
    _llm_eval_timeout_sec = int(os.getenv("LLM_EVAL_TIMEOUT_SEC", "45"))
except ValueError:
    _llm_eval_timeout_sec = 45
LLM_EVAL_TIMEOUT_SEC = max(5, min(180, _llm_eval_timeout_sec))
try:
    _llm_eval_max_tokens = int(os.getenv("LLM_EVAL_MAX_TOKENS", "1200"))
except ValueError:
    _llm_eval_max_tokens = 1200
LLM_EVAL_MAX_TOKENS = max(256, min(4096, _llm_eval_max_tokens))
...
def get_state_session_id_or_404(db: Session, state_id: UUID) -> UUID:
    session_id = (
        db.execute(
            select(SessionStateSnapshot.session_id).where(
                SessionStateSnapshot.id == state_id
            )
        )
        .scalars()
        .first()
...
def ensure_user_default_session_binding(db: Session, user: User) -> None:
    ...
    latest_session_id = (
        db.execute(
            select(SimulationSession.id).order_by(SimulationSession.created_at.desc())
        )
        .scalars()
        .first()
    )
    ...
    user.session_id = latest_session_id
...
@app.get(
    "/api/sessions",
    response_model=list[SimulationSessionRead],
...
def list_sessions(
    ...
    if not has_global_session_scope(current_user):
        if current_user.session_id is None:
            ensure_user_default_session_binding(db, current_user)
            if current_user.session_id is None:
                return []
            db.commit()
...
def update_session(
    session_id: UUID,
    payload: SimulationSessionUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    obj = db.get(SimulationSession, session_id)
...
def get_session_state_bundle(
    ...
    if snapshot_id is not None:
        ...
        if not snapshot_obj or snapshot_obj.session_id != session_id:
            raise HTTPException(
                status_code=404, detail="State snapshot not found for this session"
            )
    else:
        snapshot_obj = (
            db.execute(
...
@app.post(
    "/api/auth/login", response_model=Token, dependencies=[Depends(login_rate_limit)]
)
def login_user(payload: UserLogin, request: Request, db: Session = Depends(get_db)):
    normalized_login = normalize_login(payload.login)
    now = utcnow()
    user = (
        db.execute(select(User).where(func.lower(User.username) == normalized_login))
        .scalars()
        .first()
    )
    ...
    user.failed_login_attempts = 0
    user.lockout_until = None
    ensure_user_default_session_binding(db, user)
    ...
    access_token = create_access_token(
        data={"sub": str(user.id), "sid": str(auth_session.id)}
    )
...
def revoke_session(
    ...
    if target_session.user_id != current_user.id and not has_permission(
        current_user, "users:manage"
    ):
...
def update_user_roles(
    user_id: UUID, payload: UserRolesUpdate, db: Session = Depends(get_db)
):
...
def update_user(
    ...
    if payload.is_active is False and is_locked_admin_user(db, user_id):
        raise HTTPException(
            status_code=409,
            detail="Current admin cannot be deactivated without role transfer",
        )
...
def delete_user(user_id: UUID, db: Session = Depends(get_db)):
    if is_locked_admin_user(db, user_id):
        raise HTTPException(
            status_code=409,
            detail="Current admin cannot be deleted without role transfer",
        )
...
@app.get(
    "/api/vehicles",
    response_model=list[VehicleDictionaryRead],
    dependencies=[Depends(require_permission("vehicles:read"))],
)
def list_vehicles(db: Session = Depends(get_db)):
    return (
        db.execute(select(VehicleDictionary).order_by(VehicleDictionary.id.asc()))
        .scalars()
        .all()
    )
...
def update_vehicle(
    vehicle_id: int, payload: VehicleDictionaryUpdate, db: Session = Depends(get_db)
):
    obj = db.get(VehicleDictionary, vehicle_id)
    if not obj:
        raise HTTPException(status_code=404, detail="Vehicle not found")
...
def delete_resource_deployment(
    deployment_id: UUID,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
# --- Radio transmissions ---
@app.get(
    "/api/radio/transmissions",
    response_model=list[RadioTransmissionRead],
    dependencies=[Depends(require_permission("state:read"))],
)
def list_radio_transmissions(
    session_id: UUID | None = Query(default=None),
    ...
    assert_session_scope(current_user, target_session_id)

    rows = (
        db.execute(
            select(RadioTransmission)
            .where(RadioTransmission.session_id == target_session_id)
            .order_by(RadioTransmission.created_at.desc())
            .limit(limit)
        )
        .scalars()
        .all()
    ...
    payload: list[dict] = []
    for row in rows:
        item = RadioTransmissionRead.model_validate(row).model_dump(mode="json")
...
def resolve_llm_chat_completions_url(base_url: str) -> str:
...
def build_dispatcher_journal_context(
    ...
    *,
    ...
    selected = list(reversed(journal[:max_entries]))

    context_rows: list[dict[str, str]] = []
    for item in selected:
        text = normalize_short_text(item.get("text"), fallback="-")
        if text == "-":
            continue
        author = normalize_short_text(item.get("author"), fallback="СИСТЕМА")
        created_at = normalize_short_text(item.get("created_at"), fallback="")
        time_label = normalize_short_text(item.get("time"), fallback="")
        context_rows.append(
            {
                "author": author,
                "time": time_label,
...
def build_radio_transcript_context(
    db: Session,
    *,
    session_id: UUID,
    ...
    recent_rows = (
        db.execute(
            select(RadioTransmission)
            .where(RadioTransmission.session_id == session_id)
            .order_by(RadioTransmission.created_at.desc())
            .limit(max_transmissions * 8)
        )
        .scalars()
        .all()
    )
    ...
    for row in rows:
        tx_id = (row.transmission_id or "").strip() or str(row.id)
        item = grouped.get(tx_id)
        if item is None:
            item = {
        ...
        item["chunks"] = int(item.get("chunks", 0)) + 1
        item["duration_ms"] = int(item.get("duration_ms", 0)) + int(
            row.duration_ms or 0
        )

        transcript_text = normalize_short_text(row.transcript_text, fallback="")
        if transcript_text.startswith("[voice] сообщение без авто-расшифровки"):
            transcript_text = ""
        if transcript_text:
            parts = item["transcript_parts"]
            if transcript_text not in parts:
                parts.append(transcript_text)

    context_rows: list[dict[str, Any]] = []
    for tx_id in order[-max_transmissions:]:
        item = grouped[tx_id]
        transcript_parts = [part for part in item.get("transcript_parts", []) if part]
        transcript = " ".join(transcript_parts).strip()
        if not transcript:
            duration_ms = int(item.get("duration_ms") or 0)
            transcript = (
                f"голосовое сообщение ({duration_ms} мс)"
                if duration_ms > 0
        ...
        context_rows.append(
            {
                ...
                "channel": str(item.get("channel") or "1"),
                "sender_username": normalize_short_text(
                    item.get("sender_username"), fallback="unknown"
                ),
                "sender_role": normalize_short_text(
                    item.get("sender_role"), fallback="UNKNOWN"
                ),
                "transmission_id": tx_id,
...
def build_deployments_context(
    ...
    *,
    ...
    rows = (
        db.execute(
            select(ResourceDeployment)
            .where(ResourceDeployment.state_id == snapshot_id)
            .order_by(ResourceDeployment.created_at.asc())
        )
        .scalars()
        .all()
    ...
    for row in rows:
        ...
        kind_key = row.resource_kind.value
    ...
    return events, by_status, by_kind
...
def request_lesson_llm_evaluation(
    *,
    model_name: str,
    lesson_context: dict[str, Any],
) -> tuple[dict[str, Any] | None, str | None, dict[str, Any]]:
    endpoint = resolve_llm_chat_completions_url(LLM_EVAL_BASE_URL)
    if not endpoint:
        raise HTTPException(
            status_code=409,
            detail="LLM_EVAL_BASE_URL is not configured",
        )

    system_prompt = (
        "Ты методист МЧС и оцениваешь завершенный учебный разбор. "
        "Используй расшифровки радио, журнал диспетчера и действия по развертыванию. "
        "Верни СТРОГО один JSON-объект без markdown и без пояснений с ключами: "
        "overall_score_0_100, summary, task_execution, actions_by_user, critical_issues, recommendations."
    )
    ...
    request_payload = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": json.dumps(lesson_context, ensure_ascii=False),
            },
        ],
    }
    ...
    if LLM_EVAL_API_KEY:
        headers["Authorization"] = f"Bearer {LLM_EVAL_API_KEY}"
    ...
    try:
        with urllib_request.urlopen(request, timeout=LLM_EVAL_TIMEOUT_SEC) as response:
            raw_response_text = response.read().decode("utf-8", errors="ignore")
    except urllib_error.HTTPError as exc:
        error_text = exc.read().decode("utf-8", errors="ignore")[:1000]
        raise HTTPException(
            status_code=502,
            detail=f"LLM provider HTTP {exc.code}: {error_text or exc.reason}",
        ) from exc
    except urllib_error.URLError as exc:
        raise HTTPException(
            status_code=502,
            detail=f"LLM provider unavailable: {exc.reason}",
        ) from exc

    try:
        provider_payload = json.loads(raw_response_text)
    except Exception as exc:
        raise HTTPException(
            status_code=502,
            detail="LLM provider returned non-JSON response",
        ) from exc
    ...
    if not isinstance(choices, list) or not choices:
        raise HTTPException(
            status_code=502,
            detail="LLM provider returned response without choices",
        )
    ...
    if content is None and isinstance(first_choice.get("text"), str):
        content = first_choice["text"]

    if content is None:
        raise HTTPException(
            status_code=502,
            detail="LLM provider returned empty completion",
        )

    normalized_content = content.strip()
    parsed_json = try_parse_json_object(normalized_content)
    usage_payload = (
        provider_payload.get("usage")
        if isinstance(provider_payload.get("usage"), dict)
        else {}
    )
    return parsed_json, normalized_content, usage_payload


def build_lesson_evaluation_context(
    db: Session,
    *,
    session_obj: SimulationSession,
    snapshot: SessionStateSnapshot,
    max_radio_transmissions: int,
    max_journal_entries: int,
) -> tuple[dict[str, Any], dict[str, Any]]:
    snapshot_data = clone_json_dict(snapshot.snapshot_data)

    journal_context = build_dispatcher_journal_context(
        snapshot_data,
        max_entries=max_journal_entries,
    )
    radio_context = build_radio_transcript_context(
        db,
        session_id=session_obj.id,
        max_transmissions=max_radio_transmissions,
    )
    deployment_events, deployment_by_status, deployment_by_kind = (
        build_deployments_context(db, snapshot_id=snapshot.id)
    )

    lesson_state = clone_json_dict(snapshot_data.get("training_lesson"))
    lesson_result = clone_json_dict(snapshot_data.get("lesson_result"))

    context = {
        "session": {
            "id": str(session_obj.id),
            "status": session_obj.status.value,
            "scenario_name": session_obj.scenario_name,
            "created_at": session_obj.created_at.isoformat(),
        },
        "lesson_state": lesson_state,
        "lesson_result": lesson_result,
        "dispatcher_journal": journal_context,
        "radio_transcripts": radio_context,
        "deployments": {
            "summary_by_status": deployment_by_status,
            "summary_by_kind": deployment_by_kind,
            "events": deployment_events[-200:],
        },
    }
    stats = {
        "journal_entries": len(journal_context),
        "radio_transmissions": len(radio_context),
...
@app.get(
    "/api/lessons/evaluate",
    response_model=Optional[LessonLlmEvaluationRead],
    dependencies=[Depends(require_permission("state:read"))],
)
def get_lesson_llm_evaluation(
    session_id: UUID | None = Query(default=None),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    target_session_id = session_id or current_user.session_id
    if target_session_id is None:
        return None

    assert_session_scope(current_user, target_session_id)

    snapshot = get_latest_snapshot_for_session(db, target_session_id)
    if snapshot is None:
        return None

    snapshot_data = clone_json_dict(snapshot.snapshot_data)
    evaluation_raw = snapshot_data.get("lesson_llm_evaluation")
    if not isinstance(evaluation_raw, dict):
        lesson_result_raw = snapshot_data.get("lesson_result")
        if isinstance(lesson_result_raw, dict):
            candidate = lesson_result_raw.get("llm_evaluation")
            if isinstance(candidate, dict):
                evaluation_raw = candidate

    if not isinstance(evaluation_raw, dict):
        return None

    return LessonLlmEvaluationRead.model_validate(evaluation_raw)


@app.post(
    "/api/lessons/evaluate",
    response_model=LessonLlmEvaluationRead,
    dependencies=[Depends(require_permission("scene:write"))],
)
def evaluate_lesson_with_llm(
    payload: LessonLlmEvaluationRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    target_session_id = payload.session_id or current_user.session_id
    if target_session_id is None:
        raise HTTPException(status_code=422, detail="No target session selected")
    ...
    snapshot = get_latest_snapshot_for_session(db, target_session_id)
    if snapshot is None:
        raise HTTPException(status_code=409, detail="Session snapshot not found")

    model_name = (payload.model or LLM_EVAL_MODEL).strip()
    if not model_name:
        raise HTTPException(
            status_code=409,
            detail="LLM model is not configured (set LLM_EVAL_MODEL or send model in request)",
        )

    lesson_context, context_stats = build_lesson_evaluation_context(
        db,
        session_obj=session_obj,
        snapshot=snapshot,
        max_radio_transmissions=payload.max_radio_transmissions,
        max_journal_entries=payload.max_journal_entries,
    )

    parsed_json, response_text, usage_payload = request_lesson_llm_evaluation(
    ...
    snapshot_data = clone_json_dict(snapshot.snapshot_data)
    snapshot_data["lesson_llm_evaluation"] = evaluation_payload

    lesson_result = clone_json_dict(snapshot_data.get("lesson_result"))
    if lesson_result:
        lesson_result["llm_evaluation"] = evaluation_payload
        snapshot_data["lesson_result"] = lesson_result

    snapshot.snapshot_data = snapshot_data
    db.commit()

    return LessonLlmEvaluationRead.model_validate(evaluation_payload)
