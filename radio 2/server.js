...
/** @type {Map<import('ws').WebSocket, { id: string, name: string, role: string, channel: string, room: string, joined: boolean }>} */
const clients = new Map();
const currentSpeakersByRoom = new Map(); // room -> Map(channel -> { speakerId, talkId })
const jammedChannelsByRoom = new Map(); // room -> Set(channel)
...
function getRoomSpeakers(room) {
  let roomSpeakers = currentSpeakersByRoom.get(room);
  if (!roomSpeakers) {
    roomSpeakers = new Map();
    currentSpeakersByRoom.set(room, roomSpeakers);
  }
  return roomSpeakers;
}

function getRoomJammedChannels(room) {
  let jammed = jammedChannelsByRoom.get(room);
  if (!jammed) {
    jammed = new Set();
    jammedChannelsByRoom.set(room, jammed);
  }
  return jammed;
}

function cleanupRoomState(room) {
  const hasJoinedClientsInRoom = [...clients.values()].some((client) => client.joined && client.room === room);
  if (hasJoinedClientsInRoom) {
    return;
  }
  currentSpeakersByRoom.delete(room);
  jammedChannelsByRoom.delete(room);
}

function serializeParticipant(client) {
  const info = getRoomSpeakers(client.room).get(client.channel);
  return {
    id: client.id,
    name: client.name,
    role: client.role,
...
function broadcastJson(payload, exceptWs = null, room = null) {
  for (const [ws, client] of clients) {
    if (ws === exceptWs || ws.readyState !== ws.OPEN || !client.joined) {
      continue;
    }
...
function buildStatePayload(room) {
  const participants = [];
  for (const [, client] of clients) {
    if (client.joined && client.room === room) {
  ...
  const speakersMap = {};
  const roomSpeakers = getRoomSpeakers(room);
  for (const [ch, info] of roomSpeakers) {
    const speakerClient = findClientById(info.speakerId);
    if (speakerClient && speakerClient.room === room) {
      speakersMap[ch] = {
  ...
  const jammedChannels = Array.from(getRoomJammedChannels(room));
...
function broadcastState(room) {
  broadcastJson(buildStatePayload(room), null, room);
}

function releaseChannel(room, channel, reason, releasedByClientId = null) {
  const roomSpeakers = getRoomSpeakers(room);
  const info = roomSpeakers.get(channel);
  if (!info) {
    return;
  }

  const previousSpeaker = findClientById(info.speakerId);
  roomSpeakers.delete(channel);
...
function grantChannel(ws, client) {
  ...
  const roomSpeakers = getRoomSpeakers(room);
  let talkId = 1;
  const existing = roomSpeakers.get(channel);
  if (existing) {
...
function denyChannel(ws, room, channel) {
  const info = getRoomSpeakers(room).get(channel);
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw, isBinary) => {
    ...
    if (isBinary) {
      const info = getRoomSpeakers(liveClient.room).get(liveClient.channel);
      ...
      for (const [peerWs, peerClient] of clients) {
        ...
        if (peerClient.room === liveClient.room && peerClient.channel === liveClient.channel) {
    ...
    switch (message.type) {
      case 'hello': {
        ...
        liveClient.room = safeRoom(message.room);
        if (message.channel) {
        ...
        sendJson(ws, buildStatePayload(liveClient.room));
        broadcastState(liveClient.room);
        break;
      }
      ...
      case 'ptt_request': {
        ...
        denyChannel(ws, liveClient.room, liveClient.channel);
        break;
      }
      ...
      case 'ptt_release': {
        ...
        if (info && info.speakerId === liveClient.id) {
          releaseChannel(liveClient.room, liveClient.channel, 'manual_release', liveClient.id);
        }
        break;
      }
      ...
      case 'change_channel': {
        ...
        const info = getRoomSpeakers(liveClient.room).get(liveClient.channel);
        if (info && info.speakerId === liveClient.id) {
          releaseChannel(liveClient.room, liveClient.channel, 'channel_changed', liveClient.id);
        }
      ...
      case 'jam_channel': {
        ...
        getRoomJammedChannels(liveClient.room).add(targetChannel);
        broadcastState(liveClient.room);
        break;
      }
      ...
      case 'unjam_channel': {
        if (!liveClient.joined) return;
        const targetChannel = String(message.channel || liveClient.channel).trim();
        getRoomJammedChannels(liveClient.room).delete(targetChannel);
        broadcastState(liveClient.room);
        break;
      }
      ...
      case 'signal_ice': {
        ...
        if (target.client.room !== liveClient.room) {
          sendJson(ws, {
            type: 'error',
            code: 'peer_not_in_room',
            message: 'Целевой участник находится в другой сессии рации.'
          });
          return;
        }
  ...
  ws.on('close', () => {
    ...
    const info = getRoomSpeakers(closedClient.room).get(closedClient.channel);
    if (info && info.speakerId === closedClient.id) {
      releaseChannel(closedClient.room, closedClient.channel, 'disconnect', closedClient.id);
    }

    clients.delete(ws);
    if (closedClient.joined) {
      broadcastState(closedClient.room);
      cleanupRoomState(closedClient.room);
    }
  });
